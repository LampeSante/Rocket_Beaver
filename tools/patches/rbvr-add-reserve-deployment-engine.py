...
ROOT = Path.cwd()
ENGINES = ROOT / "programs/treasury-router/src/engines"
MOD_FILE = ENGINES / "mod.rs"
ENGINE_FILE = ENGINES / "reserve_deployment.rs"

MODULE_EXPORT = "pub mod reserve_deployment;"

if not MOD_FILE.exists():
    raise RuntimeError(f"Missing engines module: {MOD_FILE}")

if ENGINE_FILE.exists():
    raise RuntimeError(
        f"{ENGINE_FILE} already exists. Refusing to overwrite it."
    )

mod_text = MOD_FILE.read_text()

if MODULE_EXPORT in mod_text:
    raise RuntimeError(
        "reserve_deployment is already exported from engines/mod.rs"
    )

timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
backup_root = ROOT / f".reserve-deployment-engine-backup-{timestamp}"
backup_mod = backup_root / MOD_FILE.relative_to(ROOT)

backup_mod.parent.mkdir(parents=True, exist_ok=False)
shutil.copy2(MOD_FILE, backup_mod)

engine_source = r'''//! Deterministic RBVR reserve-surplus deployment logic.
//!
...
pub fn calculate_reserve_floor(
    ...
    let liquidity_floor =
        calculate_bps_share(liquidity_reference, liquidity_floor_bps)?;
...
pub fn evaluate_reserve_deployment(
    ...
    let deployable_amount = if stage == ReserveDeploymentStage::Surplus {
        ...
        let calculated =
            product / u128::from(BASIS_POINTS_DENOMINATOR);

        u64::try_from(calculated)
            .map_err(|_| ReserveDeploymentError::ArithmeticOverflow)?
    } else {
        0
    };
...
try:
    ENGINE_FILE.write_text(engine_source)

    stripped = mod_text.rstrip()

    if not stripped:
        raise RuntimeError("engines/mod.rs is unexpectedly empty")

    updated_mod = stripped + "\n" + MODULE_EXPORT + "\n"
    MOD_FILE.write_text(updated_mod)

    verification = ENGINE_FILE.read_text()
    ...
    required = [
        ...
        "pub fn calculate_reserve_floor",
        "pub fn evaluate_reserve_deployment",
        "deployment_never_breaches_floor_across_rate_matrix",
    ]

    for fragment in required:
        if fragment not in verification:
            raise RuntimeError(
                f"Reserve engine verification failed: {fragment}"
            )

    if MODULE_EXPORT not in MOD_FILE.read_text():
        raise RuntimeError("Engine module export verification failed")

    print(f"Backup created: {backup_root}")
    print(f"Created: {ENGINE_FILE.relative_to(ROOT)}")
    print("Exported: engines::reserve_deployment")
    print("Reserve Deployment Engine added successfully.")

except Exception:
    if ENGINE_FILE.exists():
        ENGINE_FILE.unlink()

    shutil.copy2(backup_mod, MOD_FILE)
    raise
