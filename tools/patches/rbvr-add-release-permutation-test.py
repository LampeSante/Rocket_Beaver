...
MARKER = "all_release_orders_reach_the_same_final_treasury_state"
...
if MARKER in text:
    raise RuntimeError("The release permutation test already exists.")

tests_marker = "#[cfg(test)]\nmod tests {"
tests_start = text.find(tests_marker)

if tests_start == -1:
    raise RuntimeError("Could not locate the release.rs test module.")

# release.rs ends with the closing brace of its #[cfg(test)] module.
module_end = text.rfind("\n}")

if module_end == -1 or module_end <= tests_start:
    raise RuntimeError("Could not locate the end of the release.rs test module.")

timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
backup_root = ROOT / f".release-permutation-backup-{timestamp}"
...
patch = r'''
    ...
    fn assert_global_accounting_invariants(treasury: &TreasuryState) {
        assert!(
            ...
            "every individual bucket must satisfy lifetime = pending + released",
        );
        ...
        assert_eq!(
            ...
            "sum of all bucket lifetime allocations must equal total allocated fees",
        );
        ...
        assert_eq!(
            ...
            lifetime_total,
            "global pending plus global released must equal global lifetime allocation",
        );
    }
    ...
    fn all_release_orders_reach_the_same_final_treasury_state() {
        ...
        assert_eq!(
            permutations.len(),
            120,
            "five release buckets must produce exactly 5! execution orders",
        );
        ...
        for (permutation_index, permutation) in permutations.iter().enumerate() {
            ...
            for bucket in permutation {
                ...
                let before_received = treasury.total_fees_received;
                let before_allocated = treasury.total_fees_allocated;

                let amount = balances_for_bucket(&treasury, *bucket).0;

                assert!(
                    amount > 0,
                    "each fixture bucket must have a positive pending balance",
                );
                ...
                assert_eq!(
                    ...
                    "release execution must never alter lifetime allocations",
                );

                assert_eq!(
                    treasury.total_fees_received,
                    before_received,
                    "release execution must never alter received-fee accounting",
                );

                assert_eq!(
                    treasury.total_fees_allocated,
                    before_allocated,
                    "release execution must never alter allocated-fee accounting",
                );
            ...
            assert_eq!(checked_pending_total(&treasury), 0);
            assert_eq!(
                checked_released_total(&treasury),
                initial_lifetime_total,
                "fully released treasury must release its complete lifetime allocation",
            );
            ...
            assert_eq!(treasury.total_fees_received, initial_received);
            assert_eq!(treasury.total_fees_allocated, initial_allocated);
...
print("Global release-order invariant test inserted successfully.")
